...
$stmt = $dbh->query("SELECT tblstudents.*,tblstudents.name as class ,tblhouse.name as house FROM tblstudents INNER JOIN tblclass ON tblstudents.class = tblclass.ID INNER JOIN tblhouse ON tblstudents.house = tblhouse.ID where tblstudents.index_no='$index_no'");
