...
$sql = "SELECT tblstudents.*, tblclass.ID as houseID,tblclass.name as class_name, tblhouse.name as house_name FROM tblstudents INNER JOIN tblclass ON tblstudents.class = tblclass.ID INNER JOIN tblhouse ON tblstudents.house = tblhouse.ID where tblstudents.index_no='$index_no'";
