...
$sql = "SELECT tblstudents.*, tblclass.ID as classID,tblclass.name as class_name, tblhouse.name as house_name,tblhouse.name as house_name FROM tblstudents INNER JOIN tblclass ON tblstudents.class = tblclass.ID INNER JOIN tblhouse ON tblstudents.house = tblhouse.ID where tblstudents.index_no='$index_no'";
...
<?php
             ...
             echo '<select name="cmdclass"  id="selectSubject" class="form-control" >';
			  echo '<option value="'.$row_student['ID'].'">'.$row_student['class_name'].'</option>';
