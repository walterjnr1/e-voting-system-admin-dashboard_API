...
if(isset($_POST['btnexport']) && !empty($_REQUEST['search']['value']))
{
  $searchQuery = '';
  if (!empty($_REQUEST['search']['value'])) {
    ...
    $fields = array('SN','Photo','Index No','fullname','sex','Programme','class','Boarding Status','Aggregate','Raw Score','Enrollment Code','JHS Attended','JHS Type','Phone','Email','Father\'s Name','Father\'s Occupation','Mother\'s Name','Mother\'s Occupation','Guardian','Residential Telephone','Admission Status','Session','House','Action');
   ...
   // $conn = mysqli_connect("localhost", "username", "password", "database_name"); // Connect to the database
    $query = "SELECT * FROM tblstudents where status ='' order by ID desc"; // Define the SQL query to fetch the data from the database
